...
| **M12** | **MCP ergonomics & agent-routing** — targeted pass on the sovereign MCP tool surface to fix *routable* misses from the 2026-06-15 claude sovereign eval (15/22). Five additive components: **(A)** sharpened tool descriptions as a probabilistic routing nudge; **(B)** new sovereign `configured_policies(firewall)` tool returning a deduped configured-rule count (`count(DISTINCT entity_id)` over the ReplacingMergeTree) — fixes the "how many policies does panosvm have" miss; **(C)** `topology_snapshot(role=…, kind=…)` filter that selects nodes **directly** from `graph_nodes FINAL` by attr (NOT the edge-derived subgraph, so isolated firewall device_inventory nodes with 0 edges are visible), edges then restricted to surviving nodes — fixes the firewall-inventory miss; **(D)** corpus follow-through in `golden/core.yaml`; **(E)** new sovereign `observed_by(identifier)` L3-provenance tool (which firewall *logged* a given endpoint's flows, via `observer_hostname` → `_short_host`) — fixes the locate-labgen miss. Both new tools classed non-shareable (`configured_policies=firewall_config`, `observed_by=security_log`) ⇒ never on the public tier. Read-only, no schema change. | ✅ Done (deployed + live-proven 2026-06-19) | `services/mcp-query/src/ssdf_mcp_query/` (`graphstore.py` `build_nodes_by_attr_sql`+`nodes_by_attr`, `topo_tools.py` `topology_snapshot` role/kind, `entitystore.py` `build_observers_for_ips_sql`/`observers_for_ips`, `access_tools.py` `observed_by`+`configured_policies`, `classification.py`, `server.py`), `services/evals/golden/core.yaml`+`corpus.py`; deployed ct106 (source sync + restart) | Branch `m12-mcp-ergonomics`. Spec `docs/superpowers/specs/2026-06-18-ssdf-m12-mcp-ergonomics-design.md`, plan `…-mcp-ergonomics.md`. 237 mcp-query + 64 evals unit tests. **Re-eval → claude sovereign 16/22** (`results/2026-06-19-…sovereign-…16_31_53Z.json`). Deterministic M12 wins over baseline: `topo-firewall-inventory`, `topo-locate-labgen`, `reach-configured-policy-count-panosvm` all flip green. **Live-found (caught only post-deploy, masked by unit stubs):** (1) `observed_by` `TYPE_MISMATCH` — `events.timestamp` is `DateTime64(3,'UTC')` and rejects a raw ISO `+00:00` String cast ⇒ wrap the window bound in `parseDateTimeBestEffort({since:String})`; (2) `topology_snapshot(role=firewall)` returned empty because `load_subgraph` derives nodes FROM edges and firewall nodes are isolated ⇒ added the direct `nodes_by_attr` graph_nodes path. **Live dependency:** paloalto ingest (hence `observed_by(panosvm-side IP)` + panos eval questions) goes stale whenever **panosvm VMID 900 is stopped** — found stopped mid-eval, operator restarted it, ingest + `observed_by(10.74.11.20)→panosvm` recovered |
...
| **M14a** | **Fleet role-rename (vSRX test → production)** — 23 logical vSRX firewalls renamed to role-based `vsrx-<role>` (all-lowercase kebab) across Proxmox VM names, Junos on-box host-names, rust-junosmcp `devices.json` keys, and SSDF (`JUNOS_DEVICES` ×3, Vector observer gate, eval corpus, docs). Standard: `docs/naming-standard.md`. panosvm unchanged. Vector observer gate dual-accepts old+new names during ~30-day transition (both `graph_nodes` + `events` TTL 30d), then legacy alternates pruned (Phase 1D follow-up). | ✅ Done (deployed + live-proven 2026-07-06) | `docs/naming-standard.md`, `infra/vector/vector.toml` (observer gate dual-accept), `services/{topo,entity,policy,health}/infra/ENV.local` (JUNOS_DEVICES), `services/evals/golden/core.yaml`; fleet (rename + Junos host-name + rust-junosmcp inventory), ct102 (Vector), ct109 (4 service envs), ct106 (mcp-query) | Branch `m14-fleet-telemetry`. Plan `~/.claude/plans/research-this-project-ssdf-ticklish-rivest.md`. **50/50 `vector test`** (dual-accept preserves old vsrx-testN). **Live:** 25 devices in `ingest_status` fresh; graph_nodes shows both old + new names during transition |
| **M14 (hotfix)** | **Dual-accept observer gate (fixes 514k/wk unattributed mnha-router + ISP-A/B)** — Vector observer gate regex broadened to catch unattributed production hosts: `^vsrx-(test\d\|isp-[ab]\|mnha-router\|production\|br\d\d)` (case-preserved for provenance bridge). nftables source range 198.51.100.219-.245. Policy+health collectors expanded from 1-2 devices to full fleet (JUNOS_DEVICES 24 fleet + panosvm). | ✅ Done (deployed + live-proven 2026-07-06) | `infra/vector/vector.toml`, `infra/firewall/ct102-ingest.nft`, ct109 service envs; deployed ct102 (nft + Vector) + ct109 (topo/policy/health/entity) | Same branch+plan. **50/50 `vector test`**. **Live:** juniper unknown=0 invariant holds; all 25 devices show in topology/policy/health/audit; ISP-A/B + mnha-router flows now carry observer_hostname |
| **M14b** | **Parser completeness (RT_SCREEN + generic RT_* fallback; PAN-OS unparsed-type fallback)** — `srx_ecs` RT_SCREEN typed parse (`event_kind=alert`, `category [network,intrusion_detection]`, `action screen_<attack>`) + generic RT_* msgid fallback (never 'unknown' for parseable sd-syslog). `panos_ecs` generic fallback for URL/WILDFIRE/DATA/TUNNEL/AUTH/DECRYPTION (was 'unknown'). **Live invariant:** juniper unknown=0. | ✅ Done (deployed + live-proven 2026-07-06) | `infra/vector/vector.toml` (`srx_ecs`+`panos_ecs` transforms); deployed ct102 (Vector) | Same branch+plan. **50/50 `vector test`** (adds RT_SCREEN suite + generic-fallback tests). **Live:** juniper unknown=0 confirmed; prior 'unknown' PAN-OS types now typed |
| **M14c** | **Junos SYSTEM syslog source (UDP 518)** — new Vector source `junos_sys_syslog` + `junos_sys_sec` filter + `junos_sys_ecs` remap (`parse_syslog` RFC5424; auth_* from SSHD_LOGIN_*/Accepted, configuration_* from UI_COMMIT/UI_CFG_AUDIT_*; UI_COMMIT_PROGRESS/UI_NETCONF_CMD/cron dropped). `event_provider=juniper`, observer gate identical to srx_ecs. nftables UDP/518. Runbook `onboarding/srx/system-syslog.md`. Fleet-wide device config applied (`set system syslog host 198.51.100.150 port 518 any info structured-data routing-instance mgmt_junos` — routing-instance REQUIRED, live-found). | ✅ Done (deployed + end-to-end live-proven 2026-07-06) | `infra/vector/vector.toml` (`junos_sys_*` + 11 tests), `infra/firewall/ct102-ingest.nft` (UDP/518), `onboarding/srx/system-syslog.md`; deployed ct102 (nft + Vector) + fleet (24 wire hosts) | Same branch+plan. **50/50 `vector test`**. **Live-found:** Vector `parse_syslog` nests RFC5424 SD under SD-ID key → SD-ID-agnostic regex extraction. **Live:** all 24 wire hosts proven (auth_success/failure + UI_COMMIT from each) |
| **M14d** | **`ingest_status` sovereign MCP tool** — per-firewall liveness (fresh/stale) with expected set = union of M4 topology firewall nodes + 7d `observer_hostnames`, so a device that stopped entirely still shows (stale, last_event null). Classed `security_log`, never public. Also scoped public-metrics volume measures + `deny_rate_index` to `event_action LIKE 'flow_%'` (was counting auth/config as flows). | ✅ Done (deployed + live-proven 2026-07-06) | `services/mcp-query/src/ssdf_mcp_query/` (`ingest_tools.py`, `server.py`), `services/public-metrics/src/ssdf_pubmetrics/` (`measures.py`, `metricstore.py`); deployed ct106 (sovereign tool) + ct109 (scoped metrics) | Same branch+plan. 271 mcp-query + 69 public-metrics unit tests. **Live:** ct106 `ingest_status` returns 25 current-fleet devices fresh (last_event <1h), expected count 25. **Live-found:** clickhouse-connect returns tz-aware datetimes → hours_since computed in SQL |
| **M15** | **ssdf-common shared library** — new `services/common` package (`ssdf_common`: mcp_client, clickhouse client_kwargs, config ConfigError/McpEndpoint/load_mcp_endpoint/env_bool, collectors registry+run_collectors). 7 services migrated via thin re-export shims (zero behavior change, 538/538 tests). Installed editable on ct109 (5 services) + ct106 + ct113; all live-verified. | ✅ Done (deployed + live-proven 2026-07-06) | `services/common/` (`ssdf_common`), 7 services' thin `config.py`/`collectors/` shims; deployed ct109 + ct106 + ct113 (editable install `/opt/src/common`, no-deps) | Branch `m14-fleet-telemetry` (shared). Plan same. 538/538 unit tests green (mcp-query 271, topo 66, entity 64, policy 41, public-metrics 69, health 27, evals 0). **Live:** ct106/ct113 `describe_schema` responsive; ct109 5-timer-driven resolvers active, logs show shared-lib imports |
| **fabric_status** | **Sovereign MCP health-check tool** — reads 9 manifest-declared subjects (5 resolvers + 4 event sources) and returns healthy boolean + per-subject freshness (last_seen, hours_since, stale boolean vs. budget_hours). Classed `security_log` (sovereign-only). Detects stalled resolvers missed by exit-0 unit logs (M6a live issue). Deployed to guest 702 (ssdf-sovereign-mcp, was ct106) at 2026-08-19 22:24:30 UTC. Live-verified: all 9 subjects returned; stall-test proved detection (stopped ssdf-health.timer, waited 16min > 0.25h budget, confirmed stale:true flip, timer restarted). **Known issue:** ssdf-entity and ssdf-policy subjects error with ILLEGAL_AGGREGATION (nested max() in SQL query) — data is fresh (entity_edges 0.05h, configured entities 0.84h verified via direct CH query) but tool query is buggy. | ✅ Done (deployed + live-verified 2026-08-19) | `services/mcp-query/src/ssdf_mcp_query/` (`fabric_manifest.py`, `fabric_tools.py`, `classification.py`, `server.py`); deployed ct702 (was ct106, sovereign MCP) | Branch `sdd/2026-08-19-ssdf-fabric-status`. Task brief `.superpowers/sdd/2026-08-19-ssdf-fabric-status/task-4-brief.md`. 290/290 unit tests. **Live:** 9 subjects (7 fresh, 2 errored), stall test flipped ssdf-health from fresh→stale in 16min wait. **Live-found:** SQL aggregation bug blocks entity/policy checks despite fresh data |
...
- **M7 — sovereignty + MCP split.** Decomposed into M7a + M7b.
  ...
  - **M7b — public MCP split.** ✅ Done (deployed + live-proven 2026-06-10; see as-built row
    ...
    **shareable views** (never base tables), reusing M7a's classification + audit (`tier="public"`).
    ...
    complete end-to-end.**
  - **Cross-cutting (M14+M15).** Fleet naming is now role-based (`docs/naming-standard.md`); the
    Vector observer gate + `ssdf.events`/`graph_nodes` carry BOTH legacy and new names until
    ~2026-08-05 (30-day TTL horizon), after which the gate's legacy alternates should be pruned
    (Phase 1D — an open follow-up). The shared library seam: cross-service helpers now live in
    `services/common` (`ssdf_common`). Both MCP tiers (ct106 sovereign + ct113 public) now
    depend on `ssdf_common` being installed editable in their venv (`pip install -e
    /opt/src/common --no-deps`).
