...
- **M7 — sovereignty + MCP split.** Decomposed into M7a + M7b.
  ...
  - **M7b — public MCP split.** A 2nd physical PUBLIC MCP process running as least-privilege
    `ssdf_public`, granted SELECT only on classification-generated **shareable views** (never base
    tables), reusing M7a's classification + audit (`tier="public"`). Do when a frontier LLM is
    actually wired. Own brainstorm/spec/plan cycle.
