...
P0 (H1+H2) **done** (PR #15, row above). Remaining findings from
`docs/security/2026-06-10-vulnerability-review.md`, ranked: **M1** query-execution timeout
(dead config — `max_execution_time` never passed to CH), **M2** rate-limit/reverse-proxy +
token rotation (esp. public ct113), **M3** audit hash-chain tamper-evidence (`007_audit.sql`
fields reserved but unimplemented), **M4** `defusedxml` for vendor XML collectors, **M5**
systemd hardening (services run as root; prioritize the two MCP units), **M6** scrub upstream
error text; then **L1–L6** defense-in-depth (TLS transport, run_sql denylist test, MCP bind
tightening, `ssdf_entity` ALTER DELETE, skip public entity_store construction, FastMCP origin
checks). None started.
