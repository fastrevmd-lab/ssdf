...
def test_build_binding_map_keys_by_segment_and_ip():
    rows = [
        _binding("198.51.100.150", "aa:aa:aa:aa:aa:aa", "fwA", "2026-06-08 10:00:00.000"),
...
def test_build_binding_map_latest_observation_wins():
    rows = [
        _binding("10.64.0.5", "aa:aa:aa:aa:aa:aa", "fwA", "2026-06-08 09:00:00.000"),
...
def test_build_binding_map_skips_missing_ip_or_mac():
    rows = [_binding("", "aa:aa:aa:aa:aa:aa", "fwA"), _binding("10.64.0.5", "", "fwA")]
    binding_map, conflict = build_binding_map(rows)
    assert binding_map == {}
    assert conflict == set()
...
def test_same_ip_different_segment_never_merges():
    flows = [
        _flow(src_ip="198.51.100.150", observer_hostname="fwA"),
        _flow(src_ip="198.51.100.150", observer_hostname="fwB", dst_ip="9.9.9.9"),
    ]
    entities, _ = resolve_entities(flows, bindings=[], tenant="t_main")
    srcs = [
        e for e in entities if e["kind"] == ASSET and e["identifiers"].get("ip") == "198.51.100.150"
    ]
...
def test_mac_known_endpoint_is_mac_anchored():
    ...
    src = next(
        e
        for e in entities
        if e["kind"] == ASSET and e["identifiers"].get("mac") == "aa:bb:cc:dd:ee:ff"
    )
...
def test_binding_only_matches_within_segment():
    ...
    entities, _ = resolve_entities(
        [_flow(observer_hostname="fwB")], bindings=bindings, tenant="t_main"
    )
    src = next(
        e for e in entities if e["kind"] == ASSET and "10.64.0.5" in e["identifiers"].values()
    )
...
def test_two_ips_sharing_a_mac_collapse_to_one_asset():
    bindings = [
        _binding("10.64.0.5", "aa:aa:aa:aa:aa:aa"),
    ...
    macs = [
        e
        for e in entities
        if e["kind"] == ASSET and e["identifiers"].get("mac") == "aa:aa:aa:aa:aa:aa"
    ]
    assert len(macs) == 1
...
def test_collapsed_mac_endpoints_share_one_comm_edge():
    ...
    flows = [
        _flow(src_ip="10.64.0.5", flows=3, bytes=1000),
...
def test_ip_conflict_sets_flag_on_mac_asset():
    bindings = [
        _binding("10.64.0.5", "aa:aa:aa:aa:aa:aa", "fw1", "2026-06-08 09:00:00.000"),
        _binding("10.64.0.5", "dd:dd:dd:dd:dd:dd", "fw1", "2026-06-08 11:00:00.000"),
    ]
    entities, _ = resolve_entities([_flow(src_ip="10.64.0.5")], bindings=bindings, tenant="t_main")
    # latest binding wins => dd:.. asset, flagged with the conflicting ip
    asset = next(
        e
        for e in entities
        if e["kind"] == ASSET and e["identifiers"].get("mac") == "dd:dd:dd:dd:dd:dd"
    )
...
def test_same_rule_name_different_vendor_is_distinct_policy():
    flows = [
        _flow(rule_name="allow-web", provider="juniper"),
...
def test_flow_stats_accumulate_across_rows_for_same_pair():
    flows = [
        _flow(flows=3, bytes=1000, ports=[443], first_seen=NOW1, last_seen=NOW1),
...
def test_observer_hosts_recorded_on_comm_edge():
    entities, edges = resolve_entities(
        [_flow(observer_hostname="vSRX-test10")], bindings=[], tenant="t_main"
    )
    comm = next(e for e in edges if e["edge_type"] == COMMUNICATED_WITH)
...
def test_observer_hosts_union_across_rows():
    ...
    flows = [
        _flow(observer_hostname="vSRX-test10", first_seen=NOW1, last_seen=NOW1),
